...
const UserPage = ({ username }) => {
    ...
    const userRank = dummyLeaderboardData.findIndex(user => user.username === username) + 1;
        ...
        >
            ...
            >
                ...
                >
                    ...
                    <p className="text-white mb-2">Your current points: <span className="font-semibold">{currentUserPoints}</span> / {overallPoints}</p>
                    <p className="text-white mb-4">Your rank: <span className="font-semibold">{userRank}</span> out of {dummyLeaderboardData.length} users.</p>
                    <ul className="text-white">
                        {dummyLeaderboardData.map((user, index) => (
                            <MotionDiv
                                ...
                                className="mb-1"
                            >
                                <span className="font-semibold">{user.username}</span>: {user.points} points
                            </MotionDiv>
                        ))}
                    </ul>
                </MotionDiv>
                ...
                >
                    <h2 className="text-2xl font-bold text-gray-300 mb-2">Language Progress</h2>
                    <p className="text-white mb-2">You are learning <span className="font-semibold">{dummyLanguagesData.totalLanguages}</span> languages:</p>
                    <ul className="text-white">
                        {dummyLanguagesData.languages.map((lang, index) => (
                            <MotionDiv
                                ...
                                className="mb-1"
                            >
                                <span className="font-semibold">{lang.name}</span>: {lang.progress}% complete
                            </MotionDiv>
                        ))}
                    </ul>
                </MotionDiv>
                ...
                >
                    ...
                    <p className="text-white mb-2">You have completed <span className="font-semibold">{dummyQuestsData.completed}</span> out of {dummyQuestsData.total} quests.</p>
                    <ul className="text-white">
                        {dummyQuestsData.quests.map((quest, index) => (
                            <MotionDiv
                                ...
                                className="mb-1"
                            >
                                <span className="font-semibold">{quest.title}</span>: {quest.status}
                            </MotionDiv>
                        ))}
                    </ul>
                </MotionDiv>
                ...
                <MotionDiv
                    ...
                    transition={{ duration: 0.5, ease: "easeInOut" }}
                >
                    <h2 className="text-2xl font-bold text-gray-300 mb-2">Recent Activities</h2>
                    <ul className="text-white">
                        {dummyProgressData.recentActivities.map((activity, index) => (
                            <MotionDiv
                                ...
                                className="mb-1"
                            >
                                <span className="font-semibold">{activity.date}:</span> {activity.activity}
                            </MotionDiv>
                        ))}
                    </ul>
                </MotionDiv>
