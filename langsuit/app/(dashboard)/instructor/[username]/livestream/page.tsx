...
const LiveStreamAnalytics = () => {
  ...
  const totalAttendees = streams.reduce((sum, stream) => sum + stream.attendees, 0);
  ...
  return (
    <div className="space-y-4" style={{ backgroundColor: darkThemeColors.background, minHeight: '100vh', padding: '20px' }}>
      <MotionCard className="w-full" variants={cardVariants} initial="hidden" animate="visible" style={{ backgroundColor: darkThemeColors.cardBackground }}>
        <CardHeader>
          <h2 className="text-2xl font-bold" style={{ color: darkThemeColors.text }}>Live Stream Analytics</h2>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <MotionCard variants={cardVariants} initial="hidden" animate="visible" style={{ backgroundColor: darkThemeColors.cardBackground }}>
              <CardContent>
                <p className="text-lg font-semibold" style={{ color: darkThemeColors.highlightText }}>Total Streams</p>
                <p className="text-3xl font-bold" style={{ color: darkThemeColors.text }}>{streams.length}</p>
              </CardContent>
            </MotionCard>
            <MotionCard variants={cardVariants} initial="hidden" animate="visible" style={{ backgroundColor: darkThemeColors.cardBackground }}>
              <CardContent>
                <p className="text-lg font-semibold" style={{ color: darkThemeColors.highlightText }}>Total Attendees</p>
                <p className="text-3xl font-bold" style={{ color: darkThemeColors.text }}>{totalAttendees}</p>
              </CardContent>
            </MotionCard>
            <MotionCard variants={cardVariants} initial="hidden" animate="visible" style={{ backgroundColor: darkThemeColors.cardBackground }}>
              <CardContent>
                <p className="text-lg font-semibold" style={{ color: darkThemeColors.highlightText }}>Avg. Attendees/Stream</p>
                <p className="text-3xl font-bold" style={{ color: darkThemeColors.text }}>{avgAttendees}</p>
              </CardContent>
            </MotionCard>
          </div>
        </CardContent>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={streams}>
              ...
              <Tooltip contentStyle={{ backgroundColor: darkThemeColors.background, border: 'none', color: darkThemeColors.text }} />
              <Line type="monotone" dataKey="attendees" stroke={darkThemeColors.lineColor} strokeWidth={3} />
            </LineChart>
          </ResponsiveContainer>
        </CardContent>
      </MotionCard>
