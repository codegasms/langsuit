...
const CoursesTable = () => {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = coursesTable.filter(
      (course) => course.title.toLowerCase().includes(term) || course.category.toLowerCase().includes(term)
    );
  ...
  const handleDeleteCourse = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCoursesTable(coursesTable.filter(course => course.id !== courseToDelete.id));
          ...
          <tbody className='divide-y divide-gray-700'>
            {(searchTerm ? filteredProducts : coursesTable).map((course) => (
              ...
              >
                ...
                <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-300'>{course.visits}</td>
                <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-300'>{course.sales || 0}</td>
                <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-300'>
