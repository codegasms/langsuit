...
import { AlertDialog, AlertDialogContent, AlertDialogCancel, AlertDialogFooter } from '@/components/ui/alert-dialog';
...
const TicketModal = ({ seat, price, isOpen, onClose, onConfirm, userId, guidanceId }) => {
...
const SeatGrid = ({
  ...
  useEffect(() => {
    const fetchBookedSeats = async () => {
      try {
        const response = await fetch('http://localhost:3000/api/ticket/booked',{
          method : "POST",
    ...
    <div className="!max-w-full mx-auto p-6 bg-gray-900 rounded-xl shadow-2xl">
      ...
      <div className="grid gap-8 perspective-1000">
        {Array.from({ length: rows }).map((_, rowIndex) => (
          <div key={rowIndex} className="grid grid-cols-6 gap-4 justify-items-center transform rotate-x-1">
              ...
              return (
                <Card
                  ...
                  className={combineClasses(
                    ...
                    isBooked ? "bg-gray-800 cursor-not-allowed overflow-hidden" : "bg-gray-700",
                    isSelected ? "ring-2 ring-amber-500 shadow-amber-500/50 shadow-lg" : "",
                      ...
                      <div className="absolute inset-0 flex justify-around">
                        {[...Array(6)].map((_, i) => (
                          <div
                            ...
                            style={{
                              backgroundImage: 'linear-gradient(0deg, transparent 0%, gray 30%, gray 70%, transparent 100%)'
                            }}
                          />
                        ))}
                      </div>
                      ...
                      <div className="absolute inset-0 flex flex-col justify-around">
                        {[...Array(6)].map((_, i) => (
                          <div
                            ...
                            style={{
                              backgroundImage: 'linear-gradient(90deg, transparent 0%, gray 30%, gray 70%, transparent 100%)'
                            }}
                          />
                        ))}
                      </div>
