...
class OverviewSection extends Section {
    ...
    async getStatCardData(): Promise<OverviewStatCardData> {
        const [totalSales, newUsers, totalCourses, conversionRate] = await Promise.all([
            this.getTotalSales(),
            this.getNewUsers(),
    ...
    private async getNewUsers(): Promise<number> {
        ...
        }).from(users).where(sql`registered_at >= ${thirtyDaysAgo}`);
        return Number(result[0]?.newUsers) || 0;
    }

    private async getTotalCourses(): Promise<number> {
        const result = await db.execute(sql`SELECT COUNT(*) AS totalCourses FROM courses`);
    ...
    private async getConversionRate(): Promise<number> {
        const [totalUsers, paidUsers] = await Promise.all([
            ...
            db.select({ count: sql`COUNT(*)` }).from(users).where(sql`has_purchased = true`)
        ]);
...
class CoursesSection extends Section {
    ...
    async getStatCardData(): Promise<CoursesStatCardData> {
        const [totalCourses, topSelling, lowestVisit, totalRevenue] = await Promise.all([
            this.getTotalCourses(),
    ...
    private async getTotalCourses(): Promise<number> {
        const result = await db.execute(sql`SELECT COUNT(*) AS total_courses FROM courses`); // Don't Know why drizzle with neon does not support .count() or raw SQL queries
    ...
    private async getTotalRevenue(): Promise<number> {
        const result = await db.execute(sql`SELECT COALESCE(SUM(amount), 0) AS totalRevenue FROM sales`);
        // console.log(result);
...
class UsersSection extends Section {
    ...
    async getStatCardData(): Promise<UsersStatCardData> {
        const [totalUsers, todayNewUsers, activeUsers, churnRate] = await Promise.all([
            this.getTotalUsers(),
    ...
    private async getTotalUsers(): Promise<number> {
        const result = await db.execute(sql`SELECT COUNT(*) AS total_users FROM users`);
    ...
    private async getChurnRate(): Promise<number> {
        const resultTotal = await db.execute(sql`SELECT COUNT(*) AS total_users FROM users`);
...
class SalesSection extends Section {
    ...
    async getStatCardData(): Promise<SalesStatCardData> {
        const [totalRevenue, avgOrderValue, conversionRate, salesGrowth] = await Promise.all([
            this.getTotalRevenue(),
    ...
    private async getAvgOrderValue(): Promise<number> {
        const result = await db.select({
            avgOrderValue: sql`COALESCE(AVG(amount), 0)::BIGINT`.as('avgOrderValue'),
        }).from(sales);
    ...
    private async getSalesGrowth(): Promise<number> {
        ...
        const [currentPeriodSales, previousPeriodSales] = await Promise.all([
            ...
            }).from(sales).where(sql`created_at >= ${thirtyDaysAgo}`),
            db.select({
                total: sql`COALESCE(SUM(amount), 0)`.as('total'),
            }).from(sales).where(sql`created_at >= ${sql`${thirtyDaysAgo}::date - INTERVAL '30 days'`} AND created_at < ${thirtyDaysAgo}`)
        ]);
...
class OverviewPieChart extends PieChart {
    async fetchData(): Promise<Array<{ name: string; value: number }>> {
        ...
        const totalCourses = result.reduce((acc, curr) => acc + Number(curr.count), 0);
...
class SalesLineChart extends LineChart {
    async fetchData(): Promise<any> {
        return await db.select({
            ...
            sales: sql`SUM(sales)`,
        }).from(courses).groupBy(sql`DATE_TRUNC('month', courses.created_at)`);
    }
}
